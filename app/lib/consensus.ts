...
function getPlacements(ranking: string[]) {
...
export function computeTakeMetrics(ranking: string[], consensus: ConsensusSnapshot): TakeMetrics {
  const userPlacements = getPlacements(ranking);
  ...
  const maxDelta = DEFAULT_RANKING.length - 1;
  const fanMatchPercent = Math.max(0, Math.round((1 - averageDelta / maxDelta) * 100));
  const takeHeat = Math.min(100, Math.round((averageDelta / maxDelta) * 100));
